        ...
        {lyrics.map((line, i) => {
          ...
          return (
            <div key={i} style={{ position: "relative", padding: "8px 0", transition: "all 0.3s ease", transform: isActive ? "scale(1.02)" : "scale(1)", transformOrigin: "left center" }}>
              <div style={{ fontSize: isActive ? 18 : isNext ? 16 : 15, fontWeight: isActive ? 700 : 500, fontFamily: "'DM Sans', sans-serif", lineHeight: 1.5, position: "relative", color: isPast ? "rgba(255,255,255,0.2)" : isActive ? "transparent" : isNext ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.18)" }}>
                {line.text}
                {isActive && <span style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, overflow: "hidden", display: "block" }}><span style={{ background: `linear-gradient(90deg, #fff ${fillPct * 100}%, rgba(255,255,255,0.35) ${fillPct * 100}%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", fontWeight: 700, fontSize: 18, fontFamily: "'DM Sans', sans-serif", lineHeight: 1.5 }}>{line.text}</span></span>}
              </div>
